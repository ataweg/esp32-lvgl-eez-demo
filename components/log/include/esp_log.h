...
// In contrast to the original, we use line numbers for LOG_SYSTEM_TIME_FORMAT here
#define LOG_FORMAT( letter, format )  LOG_COLOR_ ## letter #letter " (%u) %s:\t " format LOG_RESET_COLOR "\n"
#define LOG_LINE_FORMAT( letter, format )  LOG_COLOR_ ## letter #letter " (%u) %s(%d):\t " format LOG_RESET_COLOR "\n"
#define LOG_SYSTEM_TIME_FORMAT( letter, format )  LOG_COLOR_ ## letter #letter " (%s) %s(%d):\t " format LOG_RESET_COLOR "\n"
    ...
    */
      ...
      #define ESP_LOG_EARLY_IMPL( tag, format, log_level, log_tag_letter, ... ) do {   \
              if( _ESP_LOG_EARLY_ENABLED( log_level ) ) {                              \
                  esp_rom_printf( LOG_FORMAT( log_tag_letter, format ), esp_log_early_timestamp(), tag, ##__VA_ARGS__ ); \
              }} while( 0 )
   #elif CONFIG_LOG_TIMESTAMP_SOURCE_SYSTEM
      #define ESP_LOG_EARLY_IMPL( tag, format, log_level, log_tag_letter, ... ) do {   \
              if( _ESP_LOG_EARLY_ENABLED( log_level ) ) {                              \
                  esp_rom_printf( LOG_LINE_FORMAT( log_tag_letter, format ), esp_log_early_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); \
              }} while( 0 )
 ...
 */
   ...
   // espressif version
      ...
      // my version
      #define ESP_LOG_LEVEL(level, tag, format, ...) do {  \
              if (level==ESP_LOG_ERROR )          { esp_log_write(ESP_LOG_ERROR,      tag, LOG_SYSTEM_TIME_FORMAT(E, format), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); } \
              else if (level==ESP_LOG_WARN )      { esp_log_write(ESP_LOG_WARN,       tag, LOG_SYSTEM_TIME_FORMAT(W, format), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); } \
              else if (level==ESP_LOG_DEBUG )     { esp_log_write(ESP_LOG_DEBUG,      tag, LOG_SYSTEM_TIME_FORMAT(D, format), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); } \
              else if (level==ESP_LOG_VERBOSE )   { esp_log_write(ESP_LOG_VERBOSE,    tag, LOG_SYSTEM_TIME_FORMAT(V, format), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); } \
              else                                { esp_log_write(ESP_LOG_INFO,       tag, LOG_SYSTEM_TIME_FORMAT(I, format), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); } \
          } while(0)

        #define ESP_LOG( tag, format, ... )     { esp_log_write( ESP_LOG_ALWAYS,   tag, LOG_SYSTEM_TIME_FORMAT( _, format ), esp_log_system_timestamp(), tag, __LINE__ __VA_OPT__(,) __VA_ARGS__ ); }
   #endif //CONFIG_LOG_TIMESTAMP_SOURCE_xxx
#else // !(defined(__cplusplus) && (__cplusplus >  201703L))
   #if CONFIG_LOG_TIMESTAMP_SOURCE_RTOS
      // espressif version
      ...
      // my version
      #define ESP_LOG_LEVEL( level, tag, format, ... ) do { \
           if( level==ESP_LOG_ERROR )          { esp_log_write( ESP_LOG_ERROR,    tag, LOG_SYSTEM_TIME_FORMAT( E, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); } \
           else if( level==ESP_LOG_WARN )      { esp_log_write( ESP_LOG_WARN,     tag, LOG_SYSTEM_TIME_FORMAT( W, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); } \
           else if( level==ESP_LOG_DEBUG )     { esp_log_write( ESP_LOG_DEBUG,    tag, LOG_SYSTEM_TIME_FORMAT( D, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); } \
           else if( level==ESP_LOG_VERBOSE )   { esp_log_write( ESP_LOG_VERBOSE,  tag, LOG_SYSTEM_TIME_FORMAT( V, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); } \
           else                                { esp_log_write( ESP_LOG_INFO,     tag, LOG_SYSTEM_TIME_FORMAT( I, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); } \
       } while( 0 )

       #define ESP_LOG( tag, format, ... )     { esp_log_write( ESP_LOG_ALWAYS,   tag, LOG_SYSTEM_TIME_FORMAT( _, format ), esp_log_system_timestamp(), tag, __LINE__, ##__VA_ARGS__ ); }
   #endif // CONFIG_LOG_TIMESTAMP_SOURCE_xxx
#endif // !(defined(__cplusplus) && (__cplusplus >  201703L))
